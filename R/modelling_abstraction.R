...
#'
#' @examples
#' split_dataset(data.frame(x = 1:10))
#'
#' @export
split_dataset <- function(df) {
  n <- nrow(df)
  if (n < 2) stop("Data frame must have at least 2 rows")

  train_index <- seq_len(round(0.8 * n))
...
#'   default_payment_next_month = c(0, 1, 0, 1, 0)
#' )
#' split <- split_dataset(df)
#' prepare_knn_data(split$train, split$test, "default_payment_next_month")
#' @export
...
#' evaluate_k_values(train, test, train_labels, test_labels, k_values = 1:3)
#' @export
...
#' @examples
#' \dontrun{
#' df <- data.frame(k = 1:5, accuracy = c(0.6, 0.7, 0.75, 0.72, 0.8))
